...
import { handleCreateOrderAction } from './orders.actions';
...
function CreateOrderModal(props: IProps) {
  ...
  const handleSubmit = async () => {
    ...
    const res = await handleCreateOrderAction(newOrder);
    if (res) {
    ...
    <>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title className="text-black">Add a new Order</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group className="mb-3">
              <Form.Label className="text-black">Name</Form.Label>
              <Form.Control type="text" placeholder="Enter order name" value={name} onChange={(e) => setName(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label className="text-black">Type</Form.Label>
              <Form.Control type="text" placeholder="Enter order Type" value={type} onChange={(e) => setType(e.target.value)} />
            </Form.Group>
            <Form.Group className="mb-3">
              <Form.Label className="text-black">Price</Form.Label>
              <Form.Control type="number" placeholder="Enter order price" value={price} onChange={(e) => setPrice(Number(e.target.value))} />
            </Form.Group>
            {/* Category selection */}
            <Form.Group className="mb-3">
